...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const pricesBasedOnClientData = controllerPrices.getPricesBasedOnSheet(dataResponseDHL, clientDataSheet, weightForCalcs, zonedhl, Number.parseFloat(ffTaxes?.FFTaxes?.aerial || 11.54), Number.parseFloat(ffTaxes?.FFTaxes?.land || 19.01), validServicesDHL);
...
router.post('/estafeta', async (req, res) => {
    ...
    try {
        ...
        const dataBasedOnUserSheet = await controllerPrices.getPricesEstafetaBasedOnSheet(
            ...
            weightForCalcs,
            zone,
            Number.parseFloat(ffTaxes?.FFTaxes?.aerial || 0.1154),
            Number.parseFloat(ffTaxes?.FFTaxes?.land || 0.1901),
