    ...
    if (req.body.alto === "" || req.body.alto === undefined) {
        ...
        if (dataResponseESTAFETA.Error != "000") {
            ...
            if (zone.hasOwnProperty("error")) {
                ...
                const dataBasedOnUserSheet = await controllerPrices.getPricesEstafetaBasedOnSheet(dataResponseESTAFETA, clientDataSheet, weightForCalcs, zone, Number.parseFloat(ffTaxes?.FFTaxes?.aerial || 0.108), Number.parseFloat(ffTaxes?.FFTaxes?.land || 0.168), costoReexpedicion != "No" ? costoReexpedicion : "0", calculoSeguro)
